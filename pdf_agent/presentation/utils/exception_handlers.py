...
from pdf_agent.errors import (
    ApplicationException, DatabaseException, DataNotFoundException, Errors, ExternalServiceException, FieldException,
    ForbiddenException, ValidationException
)
from pdf_agent.presentation.utils.response import (
    get_bad_request, get_forbidden, get_method_not_allowed, get_not_found, get_response, get_server_error
)
...
async def handle_forbidden_exception(_: Request, exc: ForbiddenException) -> JSONResponse:
    return get_forbidden(errors=[exc.as_dict()])
...
async def handle_default_forbidden_exception(_: Request, exc: HTTPException) -> JSONResponse:
    return get_forbidden(errors=[{'code': Errors.FORBIDDEN_ERROR.code, 'message': Errors.FORBIDDEN_ERROR.message}])
...
async def handle_default_not_found_exception(_: Request, exc: HTTPException) -> JSONResponse:
    return get_not_found(errors=[{'code': Errors.RESOURCE_NOT_FOUND_ERROR.code,
                                  'message': Errors.RESOURCE_NOT_FOUND_ERROR.message}])


async def handle_default_method_not_allowed_exception(_: Request, exc: HTTPException) -> JSONResponse:
    return get_method_not_allowed(errors=[{'code': Errors.METHOD_NOT_ALLOWED_ERROR.code,
                                           'message': Errors.FIELD_ERROR.message}])


async def handle_external_service_exception(_: Request, exc: ExternalServiceException) -> JSONResponse:
    if exc.status_code >= 500:
        logger.exception(exc)

    return get_response(status_code=exc.status_code, message=exc.response_message or 'External Service Error',
                        errors=[{'code': exc.error.code, 'message': exc.message,
                                 'detail': exc.detail, 'errors': exc.inner_errors}])
...
async def handle_exception(_: Request, exc: Exception) -> JSONResponse:
    logger.exception(exc)
    return get_server_error(errors=[{'code': Errors.SERVER_ERROR.code, 'message': Errors.SERVER_ERROR.message,
                                     'detail': str(exc)}])
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    app.add_exception_handler(ForbiddenException, handle_forbidden_exception)  # type: ignore
    app.add_exception_handler(DataNotFoundException, handle_data_not_found_exception)  # type: ignore
    app.add_exception_handler(ExternalServiceException, handle_external_service_exception)  # type: ignore
    app.add_exception_handler(DatabaseException, handle_database_exception)  # type: ignore

    app.add_exception_handler(403, handle_default_forbidden_exception)  # type: ignore
